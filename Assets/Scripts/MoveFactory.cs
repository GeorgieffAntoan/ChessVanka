...
public class MoveFactory
{
    ...
    public List<Move> GetMoves(Piece piece, int2 position)
    {
        _piece = piece;
        _type = piece.Type;
    ...
    void _GetPawnMoves()
    {
        if (_piece.Player == Piece.playerColor.BLACK)
        {
            ...
            _GenerateMove(limit, new int2(0, 1));

            int2 diagLeft = new int2(_position.x - 1, _position.y + 1);
            int2 diagRight = new int2(_position.x + 1, _position.y + 1);
            Tile dl = null;
            Tile dr = null;
    ...
    void _GetRookMoves()
    {
        _GenerateMove(9, new int2(0, 1));
        _GenerateMove(9, new int2(0, -1));
        _GenerateMove(9, new int2(1, 0));
        _GenerateMove(9, new int2(-1, 0));
    }
    ...
    {
        int2 move;
        move = new int2(_position.x + 2, _position.y + 1);
        ...
        move = new int2(_position.x + 1, _position.y - 2);
        _CheckAndStoreMove(move);
        move = new int2(_position.x + 1, _position.y + 2);
        _CheckAndStoreMove(move);
        move = new int2(_position.x - 1, _position.y + 2);
    ...
    void _GetKingMoves()
    {
        for (int x = -1; x <= 1; x++)
        {
            for (int y = -1; y <= 1; y++)
            {
                ...
                _CheckAndStoreMove(new int2(_position.x + x, _position.y + y));
            }
        }
    }
    ...
    void _GenerateMove(int limit, int2 direction)
    {
        for (int i = 1; i < limit; i++)
        {
            int2 move = _position + direction * i;
    ...
    void _CheckAndStoreMove(int2 move)
    {
